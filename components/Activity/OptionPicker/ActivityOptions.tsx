...
function WorkoutOptions() {
  ...
  const { activities } = useSaveActivity();
  ...
  function onPress(type: string) {
    let payload = {
      selectedActivity: type as ActivityType,
      isRunning: false,
    } as StopwatchState;
    const activityIndex: number = activities.findIndex(
      (_activity: Activity) => {
        const checkSameDay = areSameDay(new Date(), new Date(_activity.date));
        return _activity.type === type && checkSameDay;
      }
    );

    if (activityIndex !== -1) {
      payload = { ...activities[activityIndex], ...payload };
    } else {
  ...
  return Object.keys(activityTypes).map((_item: string) => (
    <View
      style={[styles.card, selectedActivity === _item && styles.selectedCard]}
      key={_item}
    >
      <Button
